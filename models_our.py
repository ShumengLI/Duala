...
############################################
# LoRA and Skip-LoRA building blocks
############################################
class LoRALinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, rank=8, alpha=8, base_weight=None, base_bias=None):
        ...
        if base_weight is None:
            ...
            self.bias = nn.Parameter(base_bias.data.clone(), requires_grad=False) if bias and base_bias is not None else None
        ...
        # tag for optimizer filtering
        self.is_lora = True

    def forward(self, x):
        # x: (..., in_features)
        base = x.matmul(self.weight.t())
        delta = x.matmul(self.lora_A.t()).matmul(self.lora_B.t()) * self.scale
...
def _replace_linear_with_lora(module: nn.Module, rank: int, alpha: int, name_filter=None, replaced_names=None):
    """
    Recursively replace nn.Linear with LoRALinear when name_filter(name, mod) returns True (or for all if None).
    Collect replaced module qualified names in replaced_names list.
    """
    ...
    for child_name, child in list(module.named_children()):
        if isinstance(child, nn.Linear):
            ...
            _replace_linear_with_lora(child, rank, alpha, name_filter, replaced_names)
    return replaced_names

class SkipLoRALayer(nn.Module):
    """
    Non-linear Skip-LoRA: Activation(B @ A @ v_raw), mapping from raw input V to a target feature dimension.
    """
    ...
    def forward(self, v_raw):
        ...
        if v_raw.dim() == 3:
            ...
            v = v_raw
        out = v.matmul(self.A.t()).matmul(self.B.t()) * self.scale
        return self.act(out)
...
class BrainNetwork(nn.Module):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, clip_scale=1):
        ...
        # containers for Skip-LoRA per-layer adapters
    ...
    def enable_skip_lora(self, v_in_dim: int, activation: str = 'gelu', rank: int = 8, alpha: int = 8, include_final: bool = False):
        ...
        # alignment adapter
        self._skip_adapter_align = SkipLoRALayer(in_features=v_in_dim, out_features=self.h, rank=rank, alpha=alpha, activation=activation)
    ...
    def forward(self, x, skip_input=None, return_skip_stats=False):
        ...
        # Mixer blocks
        # optional alignment skip added before entering first block
        if self.use_skip_lora and (skip_input is not None) and hasattr(self, '_skip_adapter_align') and (self._skip_adapter_align is not None):
        ...
        for bi, (block1, block2) in enumerate(zip(self.mixer_blocks1,self.mixer_blocks2)):
            ...
            if self.use_skip_lora and (skip_input is not None) and (self._skip_adapters_block1 is not None):
                ...
                if return_skip_stats:
                    skip_stats.append((x_linear.reshape(x_linear.size(0), -1).detach(), skip_add.reshape(skip_add.size(0), -1).detach()))
            else:
        ...
        backbone = backbone_linear_out.reshape(len(x), -1, self.clip_size)
